...
#
if [ -e $base_dir/build/libs/cwl2nxf-*.jar ]; then
  CLASSPATH=`ls $base_dir/build/libs/cwl2nxf-*.jar`

  # -- append runtime libraries
  [[ ! -f "$base_dir/.launch.classpath" ]] && echo "Missing '.launch.classpath' file -- Create it by running: ./gradlew jar" && exit 1
  CLASSPATH+=":`cat $base_dir/.launch.classpath`"
  
else
  echo "Missing application libraries -- Create it by running: ./gradlew jar"
  exit 1
fi
